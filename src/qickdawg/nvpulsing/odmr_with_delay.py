...
This implementation has a delay between the laser on and acuisiton window during the microwave on, or signal, acquisition window
'''
...
class LockinODMR(NVAveragerProgram):
    ...
    required_cfg = [
        ...
        "laser_gate_pmod",
        "relax_delay_treg",
        "laser_settle_delay_treg",   # NEW: laser-on time before signal MW pulse/readout starts
        "mw_channel",
        "mw_nqz",
        "mw_gain",
        "nsweep_points",
    ...
    def body(self):
        '''
        Signal and reference acquisitions now both start after the laser has been
        on for a settle period, so neither window catches the laser turn-on transient
        '''

        # laser turns on alone first, no MW, no readout - lets it settle
        self.trigger_no_off(
            pins=[self.cfg.laser_gate_pmod],
            width=self.cfg.laser_settle_delay_treg,
            adc_trig_offset=0,
            t=0)

        # MW pulse and signal readout now start once the laser has settled
        self.pulse(ch=self.cfg.mw_channel, t=self.cfg.laser_settle_delay_treg)
        ...
        self.trigger_no_off(
            ...
            width=self.cfg.readout_integration_treg,
            adc_trig_offset=0,
            t=self.cfg.laser_settle_delay_treg)

        self.trigger_no_off(
            pins=[self.cfg.laser_gate_pmod],
            width=self.cfg.relax_delay_treg,
            adc_trig_offset=0,
            t=self.cfg.laser_settle_delay_treg + self.cfg.readout_integration_treg)
        ...
        self.trigger(
            ...
            width=self.cfg.readout_integration_treg,
            adc_trig_offset=0,
            t=self.cfg.laser_settle_delay_treg + self.cfg.readout_integration_treg + self.cfg.relax_delay_treg)
